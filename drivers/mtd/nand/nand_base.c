...
#ifdef CONFIG_TQ210
	if ((mtd->writesize == 512 && page < 512) || (mtd->writesize == 2048 && page < 256) || (mtd->writesize == 4096 && page < 128))
#else
	...
	*maf_id = chip->read_byte(mtd);
	...
	if (!type->pagesize) {
		if ( *maf_id == 0x2c)
		{
			...
			if (( dev_id == 0xd3 ) || (dev_id == 0xdc))
			{
				/* Calc pagesize */
				mtd->writesize = 1024 << (extid & 0x3);//4096;
				extid >>= 2;
				/* Calc oobsize */
				mtd->oobsize = 224;
				extid >>= 2;
				/* Calc blocksize. Blocksize is multiples of 64KiB */
				mtd->erasesize = (64 * 1024) << (extid & 0x03);//262144;
				extid >>= 2;
	 ...
	 */
	if (((*maf_id != NAND_MFR_SAMSUNG) && (*maf_id != NAND_MFR_SPANSION) && (*maf_id != NAND_MFR_MICRON)) && !type->pagesize)
