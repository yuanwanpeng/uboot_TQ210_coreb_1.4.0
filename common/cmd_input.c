...
static void sd_download_usage(void)
{
	...
	printf("[K1:] Download u-boot(u-boot.bin) to Nand.\r\n");
	printf("[K2:] Download Kernel or NK(zImage.bin) to Nand.\r\n");
	printf("[K3:] Download LOGO(logo.bin) to Nand.\r\n");
	printf("[K4:] Download UBIFS(rootfs.ubi) to Nand.\r\n");
	printf("[K5:] Download YAFFS(rootfs.bin) to Nand.\r\n");
	printf("[K6:] Return main Menu.\r\n");
	printf("Press a key on the TQ210 Board.\r\n");

	Lcd_printf(0,16*3,RGB(0,0,0xff),0,0,"======SD  download mode Menu=====");

	Lcd_printf(0,17*4,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K1:] Download bootloader(u-boot.bin) to Nand.     ");
	Lcd_printf(0,17*5,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K2:] Download Kernel or NK(zImage.bin) to Nand.   ");
	Lcd_printf(0,17*6,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,  "[K3:] Download LOGO(logo.bin) to Nand.           ");
	Lcd_printf(0,17*7,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K4:] Download UBIFS(rootfs.ubi) to Nand.          ");
	Lcd_printf(0,17*8,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,  "[K5:] Download YAFFS(rootfs.bin) to Nand.          ");
	Lcd_printf(0,17*9,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,  "[K6:] Return main Menu.                ");
...
static void sd_download_shell(void)
{
	...
	while(!input_exit)
	{
		switch (get_button_value())
		{
			...
			{
				dbg("KEY1 up!\n");
				down_bootloader("u-boot.bin");//carlos
