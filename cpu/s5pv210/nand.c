...
static void s3c_nand_reset()
{
	int i;

	NFCONT_REG &= ~(1<<1);
	NFSTAT_REG = (1<<4);
	NFCMD_REG = (unsigned char)(NAND_CMD_RESET);
	for(i=0;i<10;i++);
	while((NFSTAT_REG & 0x11) != 0x11);
	NFCONT_REG |= (1<<1);
}
...
//	uint32_t *mecc_pos = chip->ecc.layout->eccpos;
	...
	col = mtd->writesize;
#if defined(CONFIG_EVT1)
#ifdef CONFIG_TQ210		// 不跳过OOB的前12字节
	chip->cmdfunc(mtd, NAND_CMD_RNDOUT, col, -1);
#else 		// 跳过OOB的前12字节
	...
	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		...
		chip->read_buf(mtd, p, eccsize);
#ifdef CONFIG_TQ210		// 需要跳开OOB的前12字节
		chip->write_buf(mtd, chip->oob_poi+12/* + mecc_pos[0]*/+(((mtd->writesize / eccsize) - eccsteps) * eccbytes), eccbytes);
#else		// 不需要跳开OOB的前12字节
...
/********************************************************/
#endif /* CONFIG_NAND_BL1_8BIT_ECC && (CONFIG_S5PC110 || CONFIG_S5PV210) */
...
void board_nand_init(struct nand_chip *nand)
{
	...
	mafID = readb(nand->IO_ADDR_R); /* Maf. ID */
...
#if 0
			if(mafID == NAND_MFR_MICRON)
			{
				type = &nand_flash_ids[i+1];
			} 
			else
			{
...
//			puts("SLC......\n");
			...
			if ( mafID == 0x2c) {
				...
				//因为MT29F8G08ABBCA和MT29F4G08ABAEA也是4K,256KB的。
